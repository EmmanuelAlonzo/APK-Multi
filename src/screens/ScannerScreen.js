    ...
    if (!permission.granted) {
        return (
            <View style={[styles.container, { backgroundColor: colors.background }]}>
                <Text style={{ textAlign: 'center', marginBottom: 20, color: colors.text }}>Necesitamos permiso para usar la cámara</Text>
                <Button onPress={requestPermission} title="Dar permiso" color={colors.accent} />
            </View>
        );
    }
        ...
        <View style={[styles.container, { backgroundColor: 'black' }]}>
            ...
            {scanned && (
                <View style={styles.rescanContainer}>
                    <Button title={'Toca para escanear de nuevo'} onPress={() => setScanned(false)} color={colors.accent} />
                </View>
            )}
            ...
            >
                <View style={styles.centeredView}>
                    <View style={[styles.modalView, { backgroundColor: colors.card, borderColor: isDark ? '#333' : '#ddd' }]}>
                        <Text style={[styles.modalTitle, { color: colors.text }]}>Etiqueta Detectada</Text>

                        {scannedData ? (
                            <View style={[styles.dataContainer, { backgroundColor: isDark ? '#2C2C2C' : '#F0F0F0', borderLeftColor: colors.accent }]}>
                                <View style={[styles.row, { borderColor: colors.border }]}>
                                    <Text style={[styles.label, { color: colors.textSecondary }]}>SAE:</Text>
                                    <Text style={[styles.value, { color: colors.text }]}>{scannedData.sae || "---"}</Text>
                                </View>
                                <View style={[styles.row, { borderColor: colors.border }]}>
                                    <Text style={[styles.label, { color: colors.textSecondary }]}>Grado:</Text>
                                    <Text style={[styles.value, { color: colors.text }]}>{scannedData.grade || "---"}</Text>
                                </View>
                                <View style={[styles.row, { borderColor: colors.border }]}>
                                    <Text style={[styles.label, { color: colors.textSecondary }]}>Peso:</Text>
                                    <Text style={[styles.value, { color: colors.text }]}>{scannedData.weight ? `${scannedData.weight} Kg` : "---"}</Text>
                                </View>
                                <View style={[styles.row, { borderColor: colors.border }]}>
                                    <Text style={[styles.label, { color: colors.textSecondary }]}>Colada:</Text>
                                    <Text style={[styles.value, { color: colors.text }]}>{scannedData.heat || "---"}</Text>
                                </View>
                                <View style={[styles.row, { borderColor: colors.border }]}>
                                    <Text style={[styles.label, { color: colors.textSecondary }]}>Coil:</Text>
                                    <Text style={[styles.value, { color: colors.text }]}>{scannedData.bundle || "---"}</Text>
                                </View>

                                {(!scannedData.grade || !scannedData.weight) && (
                                    <View>
                                        <Text style={{ color: colors.error, marginTop: 10, fontWeight: 'bold', textAlign: 'center' }}>
                                            ⚠️ Faltan datos clave (Grado/Peso)
                                        </Text>
                                        <Text style={{ fontSize: 10, color: colors.textSecondary, marginTop: 5, textAlign: 'center' }}>
                            ...
                            <Text style={{ color: colors.text }}>Analizando...</Text>
                        )}
                        ...
                        <View style={styles.buttonContainer}>
                            ...
                            <TouchableOpacity
                                style={[styles.button, styles.buttonClose, { borderColor: colors.textSecondary }]}
                                onPress={() => {
                                    setModalVisible(false);
                                    setScanned(false);
                                }}
                            >
                                <Text style={[styles.textStyle, { color: colors.text }]}>Cancelar</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
