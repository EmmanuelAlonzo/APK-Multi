...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, ScrollView, Alert, Platform, StatusBar, KeyboardAvoidingView } from 'react-native';
...
import { saveScanToHistory, saveManualData, getManualData, getLocalSequence, saveLocalSequence } from '../utils/storage';
...
export default function ManualScreen({ navigation }) {
    ...
    const loadPersistedData = async () => {
        ...
        if (data.SAE) setSae(data.SAE);
        // We generally keep grade/heat/bundle specific to the task, but user might want SAE persisted
    };

    const handleSave = async () => {
        if (!sae || !heat || !weight) {
            Alert.alert("Error", "Por favor completa los campos requeridos (SAE, Colada, Peso)");
            return;
        }
        ...
        try {
            ...
            try {
                ...
                if (seqToUse > 999) {
                     Alert.alert("Aviso", "La secuencia llegó a 999. Por favor cambia la fecha para reiniciar.");
                     setLoading(false);
                     return;
                }
                ...
                <ScrollView contentContainerStyle={styles.form}>
                    ...
                    <View style={styles.inputGroup}>
                        ...
                        <View style={styles.gradeContainer}>
                            {['5.50', '6.00', '6.50', '7.00', '8.00', '9.00', '10.00', '12.00'].map(g => (
                                <TouchableOpacity 
                                    key={g} 
                                    style={[styles.gradeButton, grade === g && styles.gradeSelected]}
                                    onPress={() => setGrade(g)}
                                >
                    ...
                    <TouchableOpacity 
                        ...
                        disabled={loading}
                    >
                        <Text style={styles.saveButtonText}>{loading ? 'Guardando...' : 'Guardar Datos'}</Text>
                    </TouchableOpacity>
